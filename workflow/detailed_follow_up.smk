...
(FIDs,) = glob_wildcards(
    os.path.join(fastqc_in_root, "{sample,(?!Undetermined).*}_R1_001.fastq.gz")
)
...
rule bbduk_read_trim:
    ...
    output:
        bbdukRead1=os.path.join(
            OUT_ROOT, "01_readMasking", "{samples}_R1_bbduk.fastq.gz"
        ),
        bbdukRead2=os.path.join(
            OUT_ROOT, "01_readMasking", "{samples}_R2_bbduk.fastq.gz"
        ),
    log:
...
rule kraken2_read_composition_read1:
    ...
    output:
        ...
        k2Report_R1=os.path.join(
            OUT_ROOT, "03_kraken2", "{samples}.DS.R1.nt.report.kraken2"
        ),
    log:
...
rule kraken2_read_composition_read2:
    ...
    output:
        ...
        k2Report_R2=os.path.join(
            OUT_ROOT, "03_kraken2", "{samples}.DS.R2.nt.report.kraken2"
        ),
    log:
...
rule fastqc_read1:
    ...
    output:
        html=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_001_fastqc.html"
        ),
        zip=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_001_fastqc.zip"
        ),
    conda:
...
rule fastqc_read2:
    ...
    output:
        html=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R2_001_fastqc.html"
        ),
        zip=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R2_001_fastqc.zip"
        ),
    conda:
        "envs/fastqc-0.12.1.yaml"
    threads: 4
...
rule fastqc_filtered_read1:
    ...
    output:
        html=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_bbduk_fastqc.html"
        ),
        zip=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_bbduk_fastqc.zip"
        ),
    conda:
        "envs/fastqc-0.12.1.yaml"
    threads: 4
...
rule fastqc_filtered_read2:
    ...
    output:
        html=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R2_bbduk_fastqc.html"
        ),
        zip=os.path.join(
            OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R2_bbduk_fastqc.zip"
        ),
    conda:
        "envs/fastqc-0.12.1.yaml"
    threads: 4
...
rule genome_alignment_check_R1:
    ...
    output:
        bowtie2_genome=os.path.join(
            OUT_ROOT, "02_REF", "{samples}.DS.genome_alignment.bowtie2.R1.log"
        ),
    benchmark:
...
rule genome_alignment_check_R2:
    ...
    output:
        bowtie2_genome=os.path.join(
            OUT_ROOT, "02_REF", "{samples}.DS.genome_alignment.bowtie2.R2.log"
        ),
    benchmark:
...
rule multiQC_report:
    input:
        ...
        bclconvert_top_unknown=os.path.join(
            bclconvert_reports, "Top_Unknown_Barcodes.csv"
        ),
        fastqc_read1=expand(
            os.path.join(
                OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_001_fastqc.zip"
        ...
        fastqc_filtered_read1=expand(
            os.path.join(
                OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R1_bbduk_fastqc.zip"
            ),
            samples=FIDs,
        ),
        fastqc_filtered_read2=expand(
            os.path.join(
                OUT_ROOT, "fastqc_run", "fastqc", "{samples}_R2_bbduk_fastqc.zip"
            ),
            samples=FIDs,
        ...
        bowtie2_R1=expand(
            os.path.join(OUT_ROOT, "02_SILVA", "{samples}.DS.R1.bowtie2.log"),
            samples=FIDs,
        ),
        bowtie2_R2=expand(
            os.path.join(OUT_ROOT, "02_SILVA", "{samples}.DS.R2.bowtie2.log"),
            samples=FIDs,
        ),
        kraken2_R1=expand(
            os.path.join(OUT_ROOT, "03_kraken2", "{samples}.DS.R1.nt.report.kraken2"),
            samples=FIDs,
        ),
        kraken2_R2=expand(
            os.path.join(OUT_ROOT, "03_kraken2", "{samples}.DS.R2.nt.report.kraken2"),
            samples=FIDs,
        ),
        bowtie2_genome_R1=expand(
            os.path.join(
                OUT_ROOT, "02_REF", "{samples}.DS.genome_alignment.bowtie2.R1.log"
            ),
            samples=FIDs,
        ),
        bowtie2_genome_R2=expand(
            os.path.join(
                OUT_ROOT, "02_REF", "{samples}.DS.genome_alignment.bowtie2.R2.log"
            ),
            samples=FIDs,
