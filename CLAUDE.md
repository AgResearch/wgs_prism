...
| Script | Role |
|---|---|
| `run_wgs_qc` | Thin wrapper: sets `WGS_PRISM_BIN=$(pwd)`, `exec ./_run_wgs_qc -i` |
| `_run_wgs_qc` | The real driver (see below). `-i` interactive, `-r RUN` non-interactive |
| `autostart_wgs_qc` | Polls every 15 min for `RTAComplete.txt`, then calls `_run_wgs_qc -r $RUN` |

`_run_wgs_qc` does the work no Snakefile does: loads the modules, prompts for the run name, infers the instrument type via `runinfo_to_machinetype.py`, copies the run's sample sheet to `$OUT_ROOT/SampleSheet.csv`, **opens it in `vim` for hand-editing** (`OverrideCycles`, UMI settings, batch subsetting — the prompt text documents the cases), then invokes:
...
**Re-demultiplexing is blocked by design, in two independent places.** `_run_wgs_qc` exits if `$OUTPUT_ROOT/SampleSheet/bclconvert` already exists, and `run_bclconvert` refuses on its own: `--force` is deliberately **not** passed to bcl-convert. Because Snakemake pre-creates `bclconvert/` and `bclconvert/Logs/` from the declared outputs, the rule first `rmdir`s those empty shells; if real output is present the `rmdir` fails, the directory survives, and the rule aborts with an explicit message. Bypassing the wrapper does not get you a rerun — you must remove `$OUT_ROOT/SampleSheet/bclconvert` by hand.

Consequence to be aware of: the profile's `restart-times: 5` and `rerun-incomplete: True` **cannot recover a partially-written demux**. If bcl-convert dies after writing output (OOM, timeout, node failure), every retry hits the same refusal and burns all five attempts, and the `attempt`-scaled `mem_gb`/`time` escalation on that rule never gets a chance to help. Clean the directory manually and resubmit.
...
`config/pipeline_config.yaml` holds reference paths and defaults. **Its `RUN`, `IN_ROOT` and `OUT_ROOT` values are always overridden by `--config` and the checked-in values are stale** — the file says so in its own header comment. Do not assume editing `RUN:` there has any effect.

When the sequencing project rolls over (roughly annually — see the last two commits), **two files must change and there is no single source of truth**:

1. `ILLUMINA_PROJECT` in `_run_wgs_qc` (~line 15) — drives `SEQ_ROOT` and `SEQ_BCLCONVERT_ROOT`
2. `IN_ROOT` / `OUT_ROOT` in `config/pipeline_config.yaml`
...
`autostart_wgs_qc` is likewise stale: its `get_run_folder` probes `/dataset/2024_illumina_sequencing_d/active/` three times identically, while `_run_wgs_qc` now reads from `/projects/$ILLUMINA_PROJECT/run_data`. It will not find current runs without editing.
