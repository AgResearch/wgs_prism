...
Because a retry can never succeed while that guard stands, `run_bclconvert` carries **`retries: 0`** and flat (non-`attempt`-scaled) `mem_gb=96`/`time=600`. This was added when the profile still set `restart-times: 5`, which spent all five attempts on identical, doomed cluster jobs; verified against a real sample-sheet error, `grep -c "Trying to restart job"` → 0. The profile no longer sets `restart-times`, so `retries: 0` is now the same as the default — keep it explicit, because it records that one attempt is a decision rather than an omission.
